...
В случае, если remote_id должен использоваться в hex формате, 
поле REMOTE_ID_HEX становится обязательным к заполнению. 
Параметры доступов на чтение и запись, IP, MAC, remote_id являются обязательными 
для заполнения параметрами. Кроме того, необходимо проверять валидность IP. 
Результат: как минимум методы getCOMMUTATOR и saveCOMMUTATOR 
(при создании записи использовать s_incb_commutator.nextval для кода коммутатора) 
для справочника с учётом бизнес-ограничений и скрипт с примерами их использования 
...
V_COMMUNITY (доступ на чтение или запиь) и B_MODE_WRITE, который равен 1, 
если нужно проверить доступ на запись, и 0 - на чтение. check_access_comm возвращает 1, 
если доступ есть, и 0 в противном случае. Метод должен уметь выбрасывать исключения в случае,
 если коммутатора с таким IP не существует. 
...
begin 
  select -- проверка
   ...
   where pip_address = c.ip_address
     and c.b_deleted = 0;
   if ac_comm = 0 then
     raise_application_error(-20020,'IP не существует');
     end if;
...
принимающую на вход код коммутатора и возвращающую идентификатор коммутатора во внешней системе в hex формате,
если признак использования remote_id в hex = 1, и просто REMOTE_ID в противном случае. 
Если признак использования remote_id в hex = 1, но идентификатор коммутатора в hex формате пуст, 
...
begin
  select -- проверка
   ...
   where pid_commutator = c.id_commutator
     and c.b_deleted = 0;
  if ac_comm = 0 then
    raise_application_error(-20020, 'ID коммутатора нет');
  end if;
  ...
  select --
   ...
   case ac_comm
     ...
     when 1 then 
       ...
       raise_application_error(-20020, 'id неправильно');
       end case;
     end get_remote_id;*/
